...
function StandingsTable({ group, teams, matches, criteria }) {
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-pitch/10 bg-white p-4">
      <h3 className="font-display text-lg font-bold text-pitch">{GROUP_LABELS[group] ?? `Grup ${group}`}</h3>
          ...
          <tbody>
            {standings.map((s, idx) => (
              ...
              >
                ...
                <td className="px-1 py-2 text-center">{s.gd > 0 ? `+${s.gd}` : s.gd}</td>
      ...
      <p className="mt-2 text-xs text-pitch/40">
        ...
        Peringkat 1 &amp; 2 lolos ke babak semifinal (juara grup otomatis, runner-up terbaik ditentukan lintas grup).
      </p>
      {sanctioned.length > 0 && (
        <div className="mt-3 space-y-1 border-t border-red-100 pt-2">
          {sanctioned.map((m) => (
            <p key={m._docId} className="text-xs text-red-600">
              ⚠️ {teamName(teams, m.home)} vs {teamName(teams, m.away)}: poin disesuaikan panpel (
              {teamName(teams, m.home)} {m.sanction.homePoints} pts &middot; {teamName(teams, m.away)}{' '}
              {m.sanction.awayPoints} pts){m.sanction.reason && <> — {m.sanction.reason}</>}
            </p>
          ))}
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-pitch/40">
        M = Main, M = Menang, S = Seri, K = Kalah, GM = Gol Memasukkan, GK = Gol Kemasukan, SG = Selisih Gol.
      </p>
    </div>
  )
}
